...
const COUNT_LABELS: Record<string, { label: string; unit: string }> = {
  inventory: { label: '재고', unit: '개' },
  pulls: { label: '뽑기', unit: '회' },
  box_items: { label: '유저 박스', unit: '개' },
  dex_collections: { label: '도감', unit: '개' },
  exchange_products: { label: '교환상품', unit: '개' },
  confirm_exchange: { label: '교환재고', unit: '개' },
}
...
export async function deleteGachaProductAction(
  seriesId: string,
  productId: string
  ...
  if (result.image_orphaned === true && result.image_url) {
    ...
    if (fileName) {
      const { error: storageError } = await adminClient.storage
        ...
        .remove([fileName])
      if (storageError) {
        console.error('gacha product image remove failed:', storageError.message)
      }
    }
  }
